...
**结论**：隐式的墙钟优势只出现在**软盘 + 大时间步**（64×64、dt=0.1 时快
~1.5×）。随 μ_s 增大，**当前直接法的隐式每步成本反而上升**（刚性耦合使
冻结 Jacobian 更快失效→每步重分解/重启），且 μ_s≈100 时 monolithic Newton
发散/圆盘飞出背景网格域。因此**本算例中"mu_s 越大隐式墙钟越占优"不成立**
——这正是块预条件迭代求解器（常数预条件、每步成本与耦合强度近似无关）
要解决的：让隐式每步成本不再随 μ_s 上升，刚性极限下的墙钟优势才可能出现。
