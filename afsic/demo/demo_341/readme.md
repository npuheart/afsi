...
## 三维复跑（GRID=16，200 步，t=1 s）

```bash
export MESH_341=<dir>/mesh-341.xdmf OUTPUT_PATH=<dir>/ GRID=16 STEPS=200
python fsi_paralell_offline.py     # 去掉 SwanLab 调用的副本
```

- 驱动里 `swanlab_init` 在 import 时无条件调用，离线跑需要打桩；另外本仓库给
  `fsi_paralell.py` 增加了 `OUTPUT_PATH` / `MESH_341` 两个环境变量覆盖。
- **只能串行**：与二维 IBM 算例一样，标记映射不是 MPI 安全的。
- 单核实测（本机）：$8^3$ **1.1 s/步**、$16^3$ **2.8 s/步**，readme 里的
  "2 阶网格约 12 s/步" 与 $8^3\to16^3$ 的实测吻合 —— 总代价约 $O(N^5)$
  （$N^3$ 流体求解 × IBM 循环里的 $N^2$ 标记）。故 $T=10$ s（2000 步）
  在 $16^3$ 上约 6.7 h、$32^3$ 上约 27 h。
- 本次跑 200 步 / **479 s**，$t\le1$ s。结果：
  - $u_{L2}$ 由 0 增到 0.0411（$t=1$ s 时仍在增长，方腔要到几秒后才定常）；
    $p_{L2}$ 在 $t\approx0.4$ s 后稳定在 $5.6\times10^{-3}$。
  - 圆球质心由 $(0.6,0.5,0.5)$ 漂到 $(0.528,0.492,0.500)$，位移 0.073（0.37 个半径），
    方向是 $-x$（顶盖驱动的**回流支**），节点最大位移 0.107（超过半个半径），
    即圆球被携带**并发生变形**，不是纯平移。
  - 边界速度：顶盖面均值 0.78（球附近被扰动），底面 $1.6\times10^{-18}$。
  - $z$ 中心线上 $u_z\le2.5\times10^{-3}$，中平面流动基本是二维的。
- 关于"入口"：驱动内部是自洽的 —— 动壁是 `marker_up` 即 $y=1$ 面（$u_x=1$），
  其余五面无滑移，属**方腔驱动**而不是入口问题，readme 的 "inlet" 用词才是笔误。
