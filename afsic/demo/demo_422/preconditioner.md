...
### 6.5 极值 μ_s（1000→100000）——稠密 V3 迭代数仍恒定（2026-08-06，`_v3_lu_highmu.py`）

16×16 稠密 V3（鞍点稠密 LU 精确解），μ_s 扫到 100000：

| μ_s | 1000 | 5000 | 10000 | 50000 | 100000 |
|---|---|---|---|---|---|
| 迭代数 | 21 | 22 | 24 | ~30–500* | 26 |
| 收敛 | ✓ | ✓ | ✓ | 边界* | ✓ |
| 解误差 | 1.8e-12 | 1.5e-11 | 4.1e-11 | ~8.5e-9 | 3.9e-9 |

\* μ_s=50000 的"异常"是**数值假象**：真实解误差**恒定 ~8.5–9.1e-9**（多 RHS 验证），
FGMRES 残差在舍入误差 ~1e-8 处停滞，rtol=1e-8 边界上随 RHS 随机命中/未命中（30→500 次）。
**不是结构性发散**（对比 V0/lumped：-1000 次且误差 ~1）。低 μ_s 端 15→高 μ_s 端 26
是 6 个数量级内 ~1.7× 的轻微漂移（条件数/舍入所致），可视为 μ_s 无关。

**结论**：精确 $M_s^{-1}$ + 鞍点精确 LU ⇒ 迭代数对 μ_s 真正无关（到 100000 仍成立），
验证了"稠密 V3 的 μ_s 鲁棒性"不是只在小 μ_s 成立。

**50000 达不到的根因（`_cond_study.py`）——不是预条件、不是 M_s，是系统本身的条件数**：

| μ_s | 残差 \|r\|/\|b\| | 前向误差 vs LU | 后向误差 | cond(A) | eps·cond |
|---|---|---|---|---|---|
| 1000 | 3.4e-9 | 1.9e-12 | 1.7e-17 | 5.7e9 | 1.3e-6 |
| 10000 | 3.3e-9 | 2.0e-11 | 1.3e-18 | 1.6e11 | 3.6e-5 |
| 50000 | 1.2e-8 | 1.6e-8 | **7.5e-21** | **1.5e14** | 3.4e-2 |
| 100000 | 9.8e-9 | 3.8e-9 | 1.1e-20 | 2.8e13 | 6.3e-3 |

- **cond(A) 随 μ_s 暴涨**（μ_s=50000 到 1.5e14）——monolithic A 在极值 μ_s 本身病态。
- **后向误差 $=\|Ax-b\|/(\|A\|\|x\|)$ 在 μ_s=50000 是 7.5e-21，低于机器精度**——FGMRES
  解"完美满足"方程，**不是预条件失败**。
- 前向误差（vs 稠密 LU 参考）=1.6e-8 ≪ eps·cond 最坏界（3.4e-2）——两个解都到机器精度，
  只是系统病态使解本身对 RHS 敏感。
- 残差停滞 ~1e-8 是病态系统的舍入地板；μ_s=100000 反到 9.8e-9（<rtol）能收敛，说明
...
**$A_sM_{ww}^{-1}M_{wu}$ 的稀疏结构（`_print_coupling.py`，16×16 实测）**：
$M_s^{-1}$ 是 **50% 稠密**（固体质量只耦合同分量 → 两个 ns/2×ns/2 全稠密分量块）；
三元积 $C=dt\,A_{uW}M_s^{-1}M_{fs}^T$ 是 **392×392 完全稠密块**（带内 0 个精确零、100% 填满），
占 nu² 的 3.24%，**带外全零**——即"相互作用带内稠密"，不是全局稠密。数值随 μ_s 增长
（μ_s=0.1 时 max|C|=1.8e-4；μ_s=100000 时 ~180），这正是 μ_s 依赖的来源。

**lumping M_ww 的 V3（`_v3_lumpww.py`，2026-08-06）——失败**：K~ 保持精确，
仅把 p_inv 里固体块/L-U 耦合的 $M_{ww}^{-1}$ 换成 $dt\,\mathrm{diag}(M_s)^{-1}$。
X11=I 被破坏：$X_{11}=I+F_{11}A_s(M_{ww}^{-1}-M'^{-1}_{ww})M_{wu}\sim\mu_s$。
实测：μ_s≤1 时 26 次、μ_s=10 时 43 次、μ_s=100 发散。**机理：L/U 耦合必须用精确
$M_{ww}^{-1}=dt\,M_s^{-1}$**，任何集中化都在 X11 引入 $O(\mu_s)$ 误差——与 V4（集中
附加刚度）同样结论，只是位置不同（这里在耦合，V4 在附加刚度）。
> （2026-08-06 修复带块实现正负号后**重跑确认：此结论不变**——所有 lumping 实验
> （V1/V2/V4/lumped-M_ww/consistent）原本就用正确的 $K~=K-C$ 减号；lumping 失败
> 与符号无关，是精确 $M_s^{-1}\to\mathrm{diag}(M_s)^{-1}$ 的 O(μ_s) 集中误差，非符号假象。）

### 6.7 C 的组装复杂度（`_c_complexity.py`，2026-08-06，16×16 实测）

$C=dt\,A_{uW}M_s^{-1}M_{fs}^T$ 的组装，复杂度可从 O(ns³)（稠密逆）降到**几乎为零**（隐式）：

| 方法 | 2D 实测 (16×16) | 渐进复杂度 | 内存 | 3D 可行性 |
|---|---|---|---|---|
| 稠密逆 + 全矩阵（我实验的） | 41.8ms（M_s⁻¹ 7.2 + Y 7.6 + A_uW@Y 27） | O(ns³ + ns·nnz + nnz·nu_band) | O(nu²) | ✗（ns³ 爆炸） |
| 带块（只算非零 392×392）+ 稀疏因子 | 3.6ms（**11.6× 快**，内存 **3.24%**） | O(ns^1.5 一次 + nu_band·ns^1.33 + nnz·nu_band) | O(nu_band²) | 勉强 |
| **隐式算子（不组装 C）** | **0.74ms/次**，一次性只花 M_s 因子 2.7ms | O(ns^1.5) 一次 + O(nnz)/次 | O(nnz) | **✓ 唯一 3D 可行** |

**结论**：
- **最大降幅 = 根本不组装 C**，保持隐式算子 $Cv=dt\,A_{uW}(M_s^{-1}(M_{fs}^T v))$
  （a.cpp scheme 5 的 K~ 就是这么做）。"组装"只剩一次 M_s 稀疏因子（小、可复用），
  之后每次应用 = 2 稀疏 matvec + 1 次 M_s 回代。
- 若必须显式组装（2D 稠密 V3）：**只算带块**（392×392，占 nu² 3.24%）+ **稀疏因子代替
  稠密逆**，2D 实测 41.8ms→3.6ms。
- **不可约部分**：精确 $M_s^{-1}$ 的应用不能省（lumping/近似会破坏 μ_s 鲁棒，§6.2/§6.6）——
  降低的是"如何应用/存储"（隐式、稀疏因子、带块），不是"近似它"。

**已实现（2026-08-06，`immersed.py` `solve_monolithic_reduced`）**：约化方案（V3）已改为
**带块稀疏组装**——$K~=K-C$（C 精确散播在带内，带外全零，非近似）。验证（`_validate_band_reduced.py`）：

| 项 | 结果 |
|---|---|
| 解一致性（vs scheme 0 完整 3×3） | 1.9e-15 |
| K~_band vs K~_dense | **1.4e-20**（精确散播） |
| 组装耗时（16×16/Newton） | 118ms → **47ms**（2.5×） |
| 内存 | nnz 19万 vs 稠密 474万（**24×**） |
| Newton 收敛 | **二次**（7.44→3.3e-7→4.8e-14；64×64: 32.9→1.6e-6→1.2e-11） |
| 64×64 可行性 | **可行**（旧稠密 K~ 需 9GB 不可行） |

**实现要点**：band_cols = Mfsᵀ 非零列（步内冻结）；band_rows = 当前 A_uW 非零行（逐 Newton
更新）；$Y_{band}=M_s^{-1}M_{fs}^T[:,band\_cols]$（只解带列）；$C_{band}=dt\,A_{uW}[band\_rows]\cdot
Y_{band}$ 散播进 K。符号坑：**K~ = K − C**（减！），写成加会破坏二次收敛（虽然收敛点不变）。
这解除了"稠密 K~ 只能粗网格"的限制，约化方案现在可用于 64×64+。

**存储（不存逆矩阵）**：M_s⁻¹ 稠密逆**从不显式存储**（ns=7888 时 500MB 不可行）；存的是
① M_s 的**稀疏 LU 因子**（16×16 仅 23k nnz，apply 逆 = 两次三角回代）② $Y_{band}$（逆作用在
带列上，ns×nu_band，16×16 2.0MB / 32×32 6.2MB，比旧 full Y ns×nu 小 5.5×）③ $C_{band}$（带块）。
"存逆矩阵"与"存 LU 因子 + 逆作用在需要的列上"是两回事——我们只做后者。
...
Communications）。差距具体、可补。

**可发表的强项**：
- **μ_s 无关块预条件**（稠密 V3 = 精确约化算子，X11=I 机理），实测到 μ_s=100000 迭代数恒定；
- **严谨的负面结果**（lumping/伴随 PSD/普通 K 内层预条件各自失效的机制、非伴随耦合→大 μ_s
  格式问题）——负面结果+机理有独立科学价值，很少被发表；
- **方法学**（块谱分析、后向误差 1e-21、条件数归因）。

**缺口（投稿前必须补）**：
1. **GMG on K~ 未实现，是核心障碍**——GAMG 单 V-cycle 对 P2 向量失败（§4b），GMG 未实现；
   附加刚度使 K~ 的 GMG 粗算子非平凡（辅助空间 $\Pi\bar A\Pi^*$）。这是"3D 可扩展"叙事的关键。
2. **M_s 的 GMG 是伪命题**——固体小（3D 下 ~3% 体积），稀疏因子/ILU 直接精确且便宜，
   GMG 在 M_s 上无增益（"K 和 M_s 都 GMG"只有 K/K~ 半边成立）。
3. **无 3D 结果**——全部 2D；CPC 看重 3D + 可扩展性。
4. **无 h-无关性/规模扩展数据**——需网格扫描（h-independence）+ 弱/强并行扩展。
5. **无墙钟对比**——需与 a.cpp/文献 FSI 求解器同精度比墙钟。
6. **软件未成型**——研究日志 vs 可发布软件（CPC 常带代码）。

**路线**：
- **阶段 1（2D 方法短文）**：稠密 V3 + matvec 原型 + h-收敛 + 墙钟 vs a.cpp + 干净代码
  ——可作方法短文或并入更大 FSI 论文。
- **阶段 2（3D CPC 论文）**：3D 实现 + GMG on K~（辅助空间内层预条件）+ 扩展研究
  + 3D 基准——这才是 CPC 级贡献。

### 7.1 期刊定位：JCP / SISC / CMAME vs CPC（2026-08-06）

**JCP（J. Comput. Phys.）**：要**数值方法分析**（稳定性/收敛/新算法），**不要求软件交付**
——与当前内容（μ_s 鲁棒机理、负面结果、谱/条件数分析）**比 CPC 更契合**。但门槛更高：

- **novelty 定位（诚实）**：核心思想"Schur 补/附加刚度预条件 → 参数鲁棒"在 FSI 文献是**标准**的
  （Heil、Gee/Tezduyar、Crosetto/Deparis/Quateroni、Mayr/Gee）；**新颖点在 IB 特定拓扑**——
  (1,3) 弹性刚度进流体动量 ⇒ 约化算子大 μ_s **不定**（非伴随耦合）、精确约化算子预条件 μ_s 鲁棒
  到 100000、以及"lumping/伴随 PSD/普通 K 内层"各自失效的机理。这个组合是可辩护的原创贡献。
- **数学深度**：JCP 审稿人要更严的分析——X11=I 机理可严格化（P=A 块 LDU 意义下 ⇒ P⁻¹A=I+
  μ_s 无关残差）；cond(A)~μ_s^? 的增长规律可分析；h-无关性需补。
- **2D 可接受**（方法学论文常见），但 3D 演示会强很多。

**SISC（SIAM J. Sci. Comput.）**：**参数鲁棒预条件正是它的主战场**（大量
"parameter-robust preconditioner" 论文），可能比 JCP 更合适。
**CMAME**：FSI + 浸没方法强契合。
**CPC**：软件优先，需干净代码 + 3D + 扩展。

**结论**：JCP 可行但门槛高（novelty + 数学深度）；当前是强基础，仍需补 3D+GMG+扩展+
数学框定。若只做 2D 方法论，SISC/CMAME 可能比 JCP 更容易中。
