...
## 完整 T=5s 复跑（220×41，串行）

`STEPS=5000 OUTPUT_PATH=<dir>` 需写成带尾部斜杠的目录（`main.py` 直接拼接
`config["output_path"] + "velocity.xdmf"`，漏斜杠会生成 `out339velocity.xdmf` 这类文件；
事后按 XDMF 里的 h5 名重命名即可）。

- **必须串行**：`mpirun -n 8` 会在 `IBMesh` 标记广播处报
  `could not broadcast input array from shape (256,) into shape (38,)` —— IBM 映射不是 MPI 安全的。
- 耗时 812 s（5000 步，约 0.16 s/步）；输出 201 个时刻（out_interval = num_steps/200）。
- 结果：$C_d$ 在 $t\approx2$ s 后稳定在 **2.5274**（$t>2.5$ s 区间极差 2.5270–2.5299），
  $C_l$ 均值 $-0.0528$，晚期升力呈周期 0.380 s 的正弦（$\mathrm{St}=fD/U_m=0.26$），
  但振幅仅 $|C_l|$ 的 1.5% —— **尾迹基本定常、不脱落涡街**，与圆柱阻塞率 0.24
  （r=0.05 于 0.41 宽通道）相符，故 DFG 2D-3 参考值（$C_d\approx3.22$）只可作量级参考。
- 流场：$\max|u| = 2.09$ m/s（入口峰值 1.5 m/s），圆柱前缘驻点压力约 ±1 Pa。

## 详细算例研究（网格 / 标记 / 迭代次数矩阵，2026-09）

对本算例做了系统性重跑：共用出厂物理（2.2×0.41 通道、圆盘 r=0.05 @ (0.2,0.2)、
ρ=1000、μ=1、入口峰值 1.5、Δt=1e-3），跑到 t=6 s（斜坡在 t=2 s 结束，故有 4 s 稳态窗口）。
逐步日志存于站点仓库 `static/afsi/demo339-study/`，后处理脚本 `static/afsi/demo339-study.py`。

| 运行 | 网格 | 差异 | Cd（力积分，t>2.5 s） | 结果 |
|------|------|------|------------------------|------|
| `r110` | 110×21 | 更粗 | **4.3676 ± 0.0045** | 稳定 |
| `r220` | 220×41 | 出厂 | **2.5282 ± 0.0029** | 稳定 |
| `r440` | 440×82 | 更细 | — | **t=2.28 s 发散**（u_L2 冲到 1e180，之后再未稳定） |
| `disk_marks` | 220×41 | 填充圆盘标记 | 2.5282 ± 0.0029 | 与边界环**逐位相同** |
| `iter2` | 220×41 | n_iter=2 | 1.6891 ± 0.0019 | 稳定 |
| `iter20` | 220×41 | n_iter=20 | 2.5962 ± 0.0029 | 稳定 |
| `no_cyl` | 220×41 | 标记移出域外 | 0.0000 | 参照 |

**四条结论：**

1. **力积分随标记体积缩放**：Cd 比 4.3676/2.5282 = 1.73，而标记体积 ΔV_l 比
   48.50/24.54 = 1.98 —— 网格减半、ΔV_l=Δs·h 减半、扩散力积分随之变化。
   故该配置下的 Cd **不是阻力系数**，与 readme 的判断一致。
2. **细网格不是"给出不同数值"，而是直接失效**：440×82 在 t=2.28 s 跑飞。
   这不是 CFL —— 该网格对流 CFL 只有 0.30，比粗网格稳定时的 0.15 高一倍但仍很小；
   根因是 IBM 直接力冲量 ∝ ΔV_l ∝ h，网格加密使显式耦合失稳。
   Δt 减半（5e-4）可正常启动，但 440×82 下 0.14 ms 模拟/秒，跑 4 s 需约 5 小时。
3. **标记模型无关**：填充圆盘与仅边界环给出完全相同的 Cd，`disk` 模式是纯开销。
4. **迭代次数是小参数**：2 次迭代少 33% 阻力，20 次仅多 2.7%，故出厂 n_iter=10 接近收敛。

**尾迹是定常的，不是涡街**：t>2.5 s 后升力峰峰值只有 0.005（均值 −0.053），
比动压低三个量级；两种网格给出相同的回流区长度（≈1.4D）。涡量场对称且不随时间变化。
所以从中提取的 St 是数值噪声，不能当作脱落频率与 DFG 2D-3 的 St≈0.295 比较。

**与文献不可直接比的两个原因**：① 出厂入口峰值是 1.5·Um=1.5 而平均是 1.0，
而 DFG 2D-3 用 U=1.5·sin(πt/8)、峰值 1.5 且随时间变化（本算例是恒定入口）；
② 力系数归一化基准不同（本驱动用平均速度，DFG 用峰值），同一力相差 2.25 倍。

**本轮修掉的三个缺陷**：`NX`/`NY` 原先**根本没有被读取**（所以第一批"110×21"和
"440×82"其实都跑在 220×41 上）；`DFG_PROFILE=1.5` 在出厂 Δt 下不稳定（t≈1.7 s 发散）；
readme 推荐的控制体积动量平衡在这个短通道里不可用 —— 通道壁面摩擦积分约 70 N、
进出口动量通量约 37 N，而圆柱力只有 0.13 N，信号被完全淹没。
