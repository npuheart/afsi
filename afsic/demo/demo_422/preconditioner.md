...
1. **伴随一致附加刚度预条件（最值得做）**：速度块预条件用
   $\tilde K_{prec}=K+\mu_s\Delta t\,J^*K_{s0}J$（$J=M_s^{-1}M_{fs}^T$ 已有，
   $J^*=M_{fs}M_s^{-1}$，$K_{s0}$ 固体侧刚度 ~10 行新装配）。PSD 椭圆、μ_s 鲁棒、
   仅在盘附近带宽内非零（廉价）、GAMG/GMG 友好。这正是 §3/辅助空间 "$\Pi\bar A\Pi^*$"
   的正确形式——§3 失败是因为用了**非伴随** $A_{uW}$ 加 lumped $M_s^{-1}$。可治 §2
   block-LDU 在 μ_s=100 的停滞（3D/硬固体路线的关键预条件）。
...
3. **格式修正（3D 前置）**：μ_s 无关性要求耦合块伴随一致（spread 走质量加权伴随 $J^*$
   或对称化）。这是 3D/硬固体必改项，先修再谈放大 $\Delta t$（呼应 §5 旧文"先修 §3"）。
4. 约化系统作**精确**求解器只适用粗网格（$\tilde K$ 稠密）；作**预条件**（思路 1）可扩展。

> 研究工具：`_adjoint_check.py`（对比非伴随 C_demo 与伴随一致 C_adj 的 PSD/定号，跨 μ_s 扫描）。
...
- §5.4-1 伴随一致附加刚度预条件（K_s0 装配 + 速度块集成，验证 μ_s 鲁棒性）——**最高优先级**
- §5.4-3 耦合块伴随一致化（格式修正，3D 硬固体前置）
