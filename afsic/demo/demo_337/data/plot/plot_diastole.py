...
# 读取 IBAMR 参考数据（外部数据，缺文件时跳过该曲线）
try:
    df = pd.read_csv('../reference/diastole-ibamr.csv')
    has_ibamr = True
    column_data_1 = df['x']
    column_data_2 = df['Curve1']
except FileNotFoundError:
    print("[warn] 未找到 ../reference/diastole-ibamr.csv，跳过 IBAMR 曲线")
    has_ibamr = False
    column_data_1 = column_data_2 = None

initial = np.loadtxt("../reference/ideal_middle_wall.txt")
displacements = np.loadtxt("../reference/diastole-pulse-disp.txt")
displacements_afsi = np.loadtxt("../reference/diastole-afsi.txt")
