...
class IBMesh {
  public:
    IBMesh(std::array<dolfin::Point, 2> points, std::array<size_t, 2> dims, size_t order) : order(order) {
        // generate mesh
        mesh_ptr = std::make_shared<Mesh>(RectangleMesh::create(points, {dims[0], dims[1]}, CellType::Type::quadrilateral));
    ...
    std::shared_ptr<Mesh> mesh() { return mesh_ptr; }
    ...
    Index get_index(const dolfin::Point &point) const { return get_index(point.x(), point.y()); }
