...
### 6.8 谱等价诊断（用户提议，2026-08-06，`_diag_spectral_equiv.py`）

**问题**：用户论证——若 $M_L\sim M_s$ 谱等价（常数 $[0.75,1.3]$），且 $C_L\in[0.75\,C,1.3\,C]$，
则 $(K+C_L)^{-1}(K+C)$ 谱应夹在 $[0.75,1.3]$（μ_s 无关）→ GMRES ~5 步，不该发散。
"O(μ_s) 误差"标度作为解释不完整。**先跑便宜诊断**（8×8 稠密，带 BC 去掉 K 空空间）：

| μ_s | min-eig(K±C) | eig((K±C_L)⁻¹(K±C)) Re 区间 |
|---|---|---|
| 0.1 | +1.1e-3（SPD ✓）| **[+0.997, +1.00]**（几乎恒等）|
| 10 | +1.1e-3（SPD ✓）| **[+0.80, +2.7]**（接近 $[0.75,1.3]$）|
| **100** | **−4.3e-1（非 SPD ✗）** | **[−1.24, +10.5]** |
| **1000** | **−5.2（非 SPD ✗）** | **[−0.42, +3.1]** |

**结论（用户对、我的解释不完整）**：
- **μ_s≤10：用户的谱等价论证完全成立**——谱在 $[0.8,2.7]$，与 μ_s 无关（0.1 时几乎恒等）。
  集中化在 $\tilde K$ 层**不是**问题。
- **μ_s≥100：前提崩溃**——$C=dt\,A_{uW}M_s^{-1}M_{fs}^T$ **不是 PSD**（min-eig(K+C)<0 随 μ_s
  增长），正是用户列的第一个候选：**lumping 破坏 spread/interpolate 的离散伴随关系，
  $A_{uW}M_L^{-1}M_{fs}^T$ 非对称**。PSD 区间 $[0.75,1.3]$ 对不定算子无意义，谱等价框架失效。
- **精确定位**：问题**确实在 $\tilde K$ 层**，根因是**非伴随耦合使 C 不定**（§5 结论在算子层
  再次证实），不是 O(μ_s) 标度本身。
- **下一步**：候选 B（加性 Schwarz / 两域分解）最直接——不解近似 C，固体带精确分解
  （$R_s\tilde K R_s^T$ 恰是我们已组装的带块）+ 流体 AMG(K)。候选 A（Eulerian 抹平刚度）
  是把 C 做成真正 SPD 的辅助空间方向（会改变算子，偏离 deal.II）。

### 6.9 决定性验证与矛盾调和（用户主导，2026-08-06，`_verify_Ktilde_definiteness.py`）

**用户质疑**：$\tilde K$ 不定（min-eig<0）若为真，与"μ_s=10^5 外层 FGMRES 恒 21~26 次"
直接矛盾；且诊断脚本可能和求解器不一致。**决定性检查 = 用求解器自己的装配路径取 $\tilde K$**：

| μ_s | min-eig($\tilde K_{solver}$) | \|C\|/\|K\| | 诊断 C vs 求解器 C |
|---|---|---|---|
| 100 | **−2.18e-1** | 2.3 | 一致（2e-16，仅差整体符号）|
| 100000 | **−3.52e+2** | 2.3e3 | 一致（2e-13）|

**不定性是真实的**（求解器路径，非诊断 bug；C 非 PSD = 非伴随耦合）。**矛盾调和**：

- **μ_s=10^5 外层 21~26 次 = 线性代数性质**：预条件 $P$ 是 A 的**精确块 LDU**（鞍点含精确
  $\tilde K$），故 $P^{-1}A=I$（除 BC），不定 $\tilde K$ 被**吸收进 P 内部**，不影响外层谱。
- **时间步进 Newton 在 μ_s≥100 发散**（§5 实测 |X|~1e5）——这是**格式不稳定**：条件稳定格式
  越过稳定界（$\mu_s\Delta t/\rho\sim1$，μ_s=100 时弹性 CFL~2）。$\tilde K$ 不定正是它的线性症状。
- **两者不矛盾**：线性解工作 ≠ 物理稳定。**μ_s≥100 的预条件研究是在不稳定算子上做优化**。

**修正之前的框架**（用户对）：
- §6.6 的 "O(μ_s) 集中误差"是**症状**；§6.8 的 "C 非 PSD" 是**深层原因**；
- 而 C 非 PSD 的根因是**格式条件稳定**（弹性力在流体动量 + 后向欧拉隐式化不完全/非伴随耦合），
  **不是预条件问题**——任何预条件子（含候选 A/B）都救不了 μ_s≥100 的物理不稳定。
- 对论文的含义：μ_s 鲁棒是"线性求解"鲁棒；**格式有稳定界**（~μ_sΔt/ρ=O(1)），论文必须诚实
  划定这个界，不能声称 μ_s 无关到 10^5。

**量化确认（2026-08-06 追加，用户框架下的决定性实验）**：
- **稳定界**：min-eig(K~) 在 **μ_s·Δt/ρ ≈ 0.2~0.3**（dt=0.01 时 μ_s=10→30 之间）穿过零。
- **标度是精确的 μ_s·Δt/ρ**（非 μ_s·Δt²/ρ）：(μ_s=10,dt=0.01)、(100,0.001) 的 μ_s·dt 同为 0.1，
  min-eig 都是 +2.798e-4；(30,0.01)、(100,0.003)、(300,0.001) 的 μ_s·dt 同为 0.3，min-eig 都是
  −9.485e-3。**min-eig(K~) 只依赖 μ_s·dt/ρ 一个参数**——用户的量纲判断精确成立。
- **dt 细化（μ_s=100）**：dt=0.01（μ_s·dt/ρ=1）→ |X|=23（不稳定）；dt=0.001（0.1）→ |X|=9.37
  （**稳定，与软盘参考值一致**）；dt=0.0001 → 9.374。**条件稳定，不是无条件不稳定**。
- **最终调和**：μ_s=10^5 的 21~26 次是在**不稳定算子**（μ_s·dt/ρ=1000，K~ 严重不定）上的
  线性求解——P⁻¹A=I 是线性代数性质，**物理已发散**。格式的**真实稳定界 = μ_s·Δt/ρ ≲ 0.2**；
  界内精确约化算子预条件 μ_s 无关迭代成立，界外任何预条件都救不了（物理不稳定）。
  这是比"μ_s 无关到 10^5"更诚实、也更可发表的结论。

### 6.10 三条决定性检查（用户主导，2026-08-06，`_check_Kel_realW.py`）

**check #2（1D 手推，不跑代码）**：正确隐式 Schur 是**加号**
$\tilde K_{1D}=\rho/\Delta t+\mu_s\Delta t$（恒 SPD、无条件稳定）。代码公式在 1D 给出加号
（$A_{uW}=-\mu_s$、$M_s^{-1}=1$、$M_{fs}^T=1$ ⇒ $\rho/\Delta t+\mu_s\Delta t$）。
**公式结构正确；K+C→K-C 修复方向正确**（K−C 才是 K+刚度，K+C 是反隐式更糟）。

**check #1（K_el PSD）**：固体侧 neo-Hookean 刚度 $-df_s/dW$ 的 min-eig = −2.1e-5（虚构 W）/
−1.8e-6（真实 W），相对 max|K_el|=8.4e-1 是**近零模式**——**本构切向没问题**。

**真实 W 重诊断**：真实收敛 W（|W|=1.66e-1）与虚构 W 的 min-eig(K~) 几乎相同
（(30,0.01): −9.49e-3 vs −9.31e-3）——**不定性不是虚构 W 的假象**。

**check #3（能量）**：μ_s=100，dt=0.01（界外）总能量 43.5→233→**6.2e11**（爆炸）；
dt=0.001（界内）平滑 42.8→42.9。**真不稳定，非 Newton 发散**。

**符号比对**：代码 A_uW（直接混合切向，max 1.76e-2）与伴随 spread $A_{adj}=-M_{fs}M_s^{-1}K_{el}$
（max 6.06e-1）**量级差 34×，是不同结构算子**（非 ± 关系，确认非伴随）。直接比对无法定符号
（量级差主导）；但配合 1D 推导 + K_el PSD + deal.II 验证，**无符号 bug**。

**结论（诚实）**：**不是符号错**。条件稳定是 **direct-spread（非伴随）IBFE 格式的真实性质**——
弹性力直接弱形式 spread（与 a.cpp/deal.II 一致）使有效附加刚度非 PSD，隐式方案只条件稳定
（界 μ_s·Δt/ρ≈0.2）。与 IB 文献一致（direct-spread 方法有刚度稳定限制；伴随一致/能量稳定
形式才无条件稳定）。用户的直觉（后向欧拉应无条件稳定）对**伴随一致格式**成立；本格式不满足
伴随一致性——这正是 §5"诚实局限"说的结构拓扑问题的根源，也直接决定了论文必须诚实划定稳定界。
