...
# Weak form: NeoHookean PK1 stress + penalty to fix entire pipe
dVs = TestFunction(Vs)
PK1 = Material.first_piola_kirchhoff_stress_v1(structure, solid_coords)

# Penalty constraint: fix whole solid domain to original position
...
L_hat = form(
    # -inner(PK1, grad(dVs)) * dx
    - beta * inner(displacement_constraint, dVs) * dx
)
