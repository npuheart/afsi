...
def load_mesh():
    ...
    mesh["pressure"] = np.asarray(mesh["p_"], dtype=float).ravel()
...
def make_individual(t, mesh, streamlines, seeds):
    ...
    pl.add_text(f"closed_NY45   t = {t:g} s   velocity magnitude", position="upper_left")
    view_2d(pl)
    ...
    pl.add_text(f"closed_NY45   t = {t:g} s   pressure", position="upper_left")
    view_2d(pl)
    ...
    pl.add_text(f"closed_NY45   t = {t:g} s   velocity vectors (every 40th node)", position="upper_left")
    view_2d(pl)
    ...
    pl.add_text("closed_NY45   streamlines (red: seed line at x = 1 mm)", position="upper_left")
    view_2d(pl)
    ...
    pl.add_text("closed_NY45   cross-section x = 0.05 m", position="upper_left")
    view_2d(pl)
...
def main():
    t, mesh = load_mesh()
    print(f"Read closed_NY45: t={t:g}, {mesh.n_points} points, {mesh.n_cells} cells")
