...
- **10s 完整公转（算例成功，64×64，mu_s=0.2/rho_s=1/粘性0.01）**：圆盘被主涡带着
  顺时针**完整公转一圈（周期≈5s）**，0→5s 被带到顶盖（顶边最高 **0.993**，不穿
  过），5→7s 被回流带下，8→10s 从左侧再次转上；全程无单元翻转（det_min>0.7），
  变形在贴顶时最大（|disp|max=0.73）、离顶后回弹。图见 `plot/df64_10s.png`。
- **10s 复跑（本仓库，64×64，4000 步，同样参数，2026-09 重跑）**：全程 4000 步无
  NaN，$\min\det\mathbf F=0.7038$（无单元翻转），固体体积膨胀 ~1.4%
  （$\pi r^2=0.12566\to0.1274$）。位移（P2 自由度）从 0 单调增到
  $|u_s|_{\max}=0.820$；输出到 P1 场（`solid.xdmf`，433 节点）后在
  $t=0,2,4,6,8,10$ s 处的峰值为 $0.00,0.25,0.71,0.61,0.31,0.44$，即**变形在
  $t\approx4$ s 达到最大后部分回弹**，与上面"贴顶时最大、离顶后回弹"一致。
  流体 $u_{L2}\to0.0623$（刚体版同条件为 0.0529，无固体参照 t=1.5s 为 0.0612）。
...
> 注：`solid.xdmf` 存的是**参考网格 + 位移场**（ParaView 需 Warp by Vector 查看真实
> 位置）。下面轨迹分析均用各 run 自己的 `solid.h5:/Mesh/mesh/geometry` 参考几何 +
> 位移重构，与 ParaView Warp 完全一致。

**实测（$\rho_s=1/\mu_s=0.2$，顶边 y 为真实 Warp 后位置）：**

| 量 | DF@128 无粘性 | DF@128 粘性0.01 | DF@64 粘性0.01 | IB@64 |
|----|---------------|-----------------|----------------|-------|
| 顶边>0.99 | t=4.40s | t=4.51s | t=4.54s | 未达（末 0.980） |
| 顶边 @5s | —（4.89s 时 0.978） | **0.993** | **0.993** | **0.980** |
| 质心 @5s | — | 0.863 | 0.862 | 0.789 |
| 结局 | **t=4.89s 翻转 NaN** | 5s 完成 | 5s 完成 | 5s 完成 |

**结论：**
- **所有方法圆盘都留在腔内、不刺穿顶壁**（顶边最高 0.993 < 1.0），只是贴近顶盖
  （与顶盖间隙 ~7 mm）。修正了早期"顶边 1.36 刺穿"的错误结论（那是轨迹分析时
  参考网格点序配对错误产生的假值）。
- **DF@64 ≈ DF@128**：对背景网格分辨率不敏感。
- **$\mu_s^{\text{visc}}=0.01$ 对圆盘整体运动几乎无影响**（轨迹重合，顶边差异
  <0.01）。原因：固体粘性阻尼的是**内部变形速率**（偏量应变率），而圆盘主要被主涡
  **近似刚体地平动/公转**，几乎不产生应变率 → 粘性应力≈0 → 不减速。其真正作用是
  **稳定性**：无粘性在 t=4.89s 单元翻转出 NaN（$\det F\to0$，软体在顶盖附近被持续
  剪切拉伸所致），加粘性后撑过 4.89s 跑满 5s（det_min 仍 0.70，未翻转）。
- **IB 与 DF 的差异不大但真实存在**：DF 圆盘最终质心更高（0.86 vs 0.79）、顶边更贴
  近顶盖（0.993 vs 0.980）。来源更可能是固体推进方式（IB 运动学无质量平流 vs DF 带
  惯性动量方程 + added-mass），而非网格分辨率或 0.01 粘性。
...
## 详细算例研究（6 组配置 × 3 套网格，逐步日志）

为把本算例"跑透"，做了一轮参数/网格研究，全部 $T=10$ s、$\Delta t=0.0025$ s（4000 步），
共用同一方腔与圆盘。每次运行现在都会写 `metrics.csv`
（`t,cx,cy,disp_max,disp_rms,det_min,volume,Fx,Fy,p_L2,u_L2`），由
`static/afsi/demo336-study.py` 后处理。原始逐步日志存档在
`static/afsi/demo336-study/`。

| 运行 | 参数 | 结果要点 |
|------|------|----------|
| `elastic_base` | $\mu_s=0.2,\rho_s=1$ | 质心 $x\in[0.269,0.723]$、$y\in[0.489,0.873]$；10 s 内扫过 $-587^\circ$，**转两圈**（周期≈5 s） |
| `elastic_soft` | $\mu_s=0.05$ | 路径略短；$\min\det F=0.776$（比基准更安全） |
| `elastic_heavy` | $\rho_s=20$ | t=5.595 s 触壁优雅终止（默认 `clamp_solid=False`）；|F_x| 达 0.235，约为基准的 4 倍 |
| `elastic_heavy_clamp` | $\rho_s=20$ + 质心钳位 | 钳位后轨迹变成起点附近的小环 —— **这是钳位的产物，不是重固体物理** |
| `no_solid` | 纯方腔 | $u_{L2}=0.0672$ 参照 |
| `elastic_32/64/128` | 基准参数三套网格 | 64² 与 128² **逐位一致**；32² 场量对到 0.4% 但**少转一圈**（只覆盖 $-228^\circ$） |

关键结论：

- **变形量级**：圆盘半径 0.2，节点最大位移 **0.820**（= 4 个半径），RMS 0.40 ——
  不是"轻微晃动"，而是被涡拉伸的软体。
- **面积变化**：$\lambda_s=0.5$ 属高度可压缩，$V/V_0$ 达 **1.107**（有效半径 +5.2%），
  这是材料参数的物理后果，不是守恒失效。
- **网格收敛**：$64^2$ 起场量已收敛；但**长期公转相位在 $32^2$ 上没解出来** ——
  这是本算例真正的分辨率要求。
- **能量收支**：相对纯方腔，弹性盘只拿走 7.3%（软的反而 11.8%），而钳位重盘拿走 23.0%、
  刚体盘 21.2%。轻软体确实几乎不挡流，**质量才是决定 back-effect 的参数**。
- 运行注意：本环境下 MPI 会让该算例堆损坏（heap corruption）而中止，故整轮研究都是串行；
  128² 串行 1439 s、64² 串行 521 s。
