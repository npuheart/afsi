# NOTE: When the kappa is large, the result is not good because of the oscilating. but the fitting is good. 
...
# 仅对 [0.65, 0.60, 0.3, 0.3] 范围内的点进行拟合
# x_min, x_max = -10, -8
# y_min, y_max = -10, 5
x_min, x_max = -8.75, -8.25
y_min, y_max = -2, 2
mask = (deformed_afsi[:, 1] >= x_min) & (deformed_afsi[:, 1] <= x_max) & (deformed_afsi[:, 0] >= y_min) & (deformed_afsi[:, 0] <= y_max)
print(f"Number of points used for fitting: {np.sum(mask)}")
fit_coeffs = np.polyfit(deformed_afsi[mask, 0], deformed_afsi[mask, 1], deg=2)
fit_y = np.linspace(y_min, y_max, 100)
fit_x = np.polyval(fit_coeffs, fit_y)
# sub_ur.plot(fit_x, fit_y, color='green', linestyle='-', linewidth=2, label='Fit')
sub_ur.plot(fit_x, fit_y, alpha=1.0, label='Present')
