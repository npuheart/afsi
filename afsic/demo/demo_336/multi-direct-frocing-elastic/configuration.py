...
config = {
    ...
    # --- 弹性本构（可压缩 neo-Hookean） ---
    # 原始 demo_336 用 mu_s=0.1, lambda_s=10（近不可压缩、很硬）。但"无质量弹性
    # 固体 + 显式耦合"在硬/近不可压缩时稳定窗口很短（实测 t≈0.1~0.4s 即固体网格
    # 翻转 det F→0 出 NaN，仓库对 ibfe 已记录此固有失稳）。这里默认取较软的
    # mu_s=0.02, lambda_s=0.2：稳定窗口更长（实测 >1.2s）且变形更明显；若要还原
    # 原始刚度可改回 0.1/10.0（需缩短 T，见 readme）。
    "mu_s": 0.02,          # 剪切模量（Lame μ）
    "lambda_s": 0.2,       # 第一 Lame 参数 λ
